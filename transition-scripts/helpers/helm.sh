...
KEYCLOAK_HELM_CHART_VERSION="v1.16.6"
...
upgrade_helm_standby() {
  ...
  api_token_url=$(kubectl get secret otp-credentials -n "$namespace" -o jsonpath='{.data.apiTokenUrl}' | base64 -d)
  api_url=$(kubectl get secret otp-credentials -n "$namespace" -o jsonpath='{.data.apiUrl}' | base64 -d)
  client_id=$(kubectl get secret otp-credentials -n "$namespace" -o jsonpath='{.data.clientID}' | base64 -d)
  client_secret=$(kubectl get secret otp-credentials -n "$namespace" -o jsonpath='{.data.clientSecret}' | base64 -d)
  otp_issuer=$(kubectl get secret otp-credentials -n "$namespace" -o jsonpath='{.data.otpIssuer}' | base64 -d)
